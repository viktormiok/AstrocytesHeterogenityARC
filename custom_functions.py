...
def plot_umap_marker(adata, markers, color_map='RdGy_r', size=3, save=None, use_raw=True,
                     legend_loc='right margin', palette=None, show=None, ax=None):
    for i in range(len(markers) // 2 + len(markers) % 2):
...
def cell_percent(adata, cluster, condition, norm='index', xlabel='cell cluster', ylabel='cell count',
    ...
    tmp.plot.bar(stacked=True).legend(loc=leg_loc)
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    if save is not None:
